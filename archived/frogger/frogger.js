...
function draw(ctx) {
	...
	//header image
	ctx.drawImage(sprite,0,0,399,110,0,0,399,110);
	//pathway images
	ctx.drawImage(sprite,0,119,399,34,0,282,399,34);
	ctx.drawImage(sprite,0,119,399,34,0,490,399,34);
	drawLilyPads();
	drawLives();	
	drawLogs();
	drawFly();
...
function moveLogs() {
	...
	if (vars.log[0].pX < -365) {
		vars.log[0].pX = 399;	//right edge of canvas
	}
	else {
	...
	if (vars.log[2].pX < -280) {
		vars.log[2].pX = 399;	//right edge of canvas
	}
	else {
...
function moveCars() {
	...
	if (vars.car[0].pX < -200) {
		vars.car[0].pX = 399;	//right edge of canvas
	}
	else {
	...
	if (vars.car[2].pX < -240) {
		vars.car[2].pX = 399;	//right edge of canvas
	}
	else {
